...
namespace Application.Api.ReportApiControllers
{
    ...
    public class ReportReceivePaymentAgainstSaleController : ControllerBase
    {
        ...
        public ReportReceivePaymentAgainstSaleController(IUnitOfWork unitOfWork, IWorkContext workContext, ITenantService tenantService)
        {
        ...
        [HttpGet("{receivePaymentAgainstSaleId}")]
        public async Task<IActionResult> ReceivePayments(Guid receivePaymentAgainstSaleId)
        {
            // ==============================
            ...
            headerPage.AddCell(new PdfPCell(new Phrase("Cash Received Voucher (MR)", fontArial14Bold)) { Border = 0, PaddingTop = 3f, HorizontalAlignment = Element.ALIGN_CENTER });
            ...
            spaceTable.AddCell(new PdfPCell(new Phrase(" ", fontArial10Bold)) { Border = 0, PaddingTop = 4f });
            ...
            var receivePaymentAgainstSale = _unitOfWork.Repository<ReceivePaymentAgainstSale>().TableNoTracking().Include(x => x.Customer).Include(x => x.CostCenter).Include(x => x.ToAccount).Where(d => d.Id == receivePaymentAgainstSaleId).FirstOrDefault();

            if (receivePaymentAgainstSale is not null)
            {
                String? code = receivePaymentAgainstSale.Code;
                String? customerCode = receivePaymentAgainstSale.Customer.Code;
                String? customerName = receivePaymentAgainstSale.Customer.Name;
                String? bankCode = receivePaymentAgainstSale.ToAccount.Code;
                String? bankName = receivePaymentAgainstSale.ToAccount.Name;
                String? costCenterName = receivePaymentAgainstSale.CostCenter.Name;
                String? paymentDate = receivePaymentAgainstSale.PaymentDate.ToLocal().ToString("dd/MM/yyyy");
                Decimal? amount = receivePaymentAgainstSale.Amount;
                String? remarks = receivePaymentAgainstSale.Remark;
                String? preparedBy = receivePaymentAgainstSale.CreatedBy;
                String? checkedBy;
                if (!string.IsNullOrEmpty(receivePaymentAgainstSale.CheckedBy))
                    checkedBy = receivePaymentAgainstSale.CheckedBy;
                else
                    checkedBy = "";
                String? approvedBy;
                if (!string.IsNullOrEmpty(receivePaymentAgainstSale.ApprovedBy))
                    approvedBy = receivePaymentAgainstSale.ApprovedBy;
                else
                    approvedBy = "";
                ...
                tableReceivePaymentItems.AddCell(new PdfPCell(new Phrase("1", fontArial9)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 3f, PaddingLeft = 5f, PaddingRight = 5f });
                tableReceivePaymentItems.AddCell(new PdfPCell(new Phrase(bankName, fontArial9)) { HorizontalAlignment = 0, PaddingTop = 3f, PaddingBottom = 3f, PaddingLeft = 5f, PaddingRight = 5f });
                tableReceivePaymentItems.AddCell(new PdfPCell(new Phrase(bankCode, fontArial9)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 3f, PaddingLeft = 5f, PaddingRight = 5f });
                tableReceivePaymentItems.AddCell(new PdfPCell(new Phrase(amount?.ToString("#,##0.00"), fontArial9)) { HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 3f, PaddingLeft = 5f, PaddingRight = 5f });
                ...
                tableReceivePaymentItems.AddCell(new PdfPCell(new Phrase("2", fontArial9)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 3f, PaddingLeft = 5f, PaddingRight = 5f });
                ...
                tableReceivePaymentItems.AddCell(new PdfPCell(new Phrase(amount?.ToString("#,##0.00"), fontArial9Bold)) { HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 3f, PaddingLeft = 5f, PaddingRight = 5f });
                tableReceivePaymentItems.AddCell(new PdfPCell(new Phrase(amount?.ToString("#,##0.00"), fontArial9Bold)) { HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 3f, PaddingLeft = 5f, PaddingRight = 5f });
                ...
                calculationTable.AddCell(new PdfPCell(new Phrase(NumberToWords.ConvertAmount(amount ?? 0), fontArial9)) { Border = 0, PaddingTop = 2f, PaddingLeft = 5f, PaddingRight = 5f, HorizontalAlignment = 0 });
