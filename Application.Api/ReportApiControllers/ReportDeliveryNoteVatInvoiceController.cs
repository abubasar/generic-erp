...
namespace Application.Api.ReportApiControllers
{
    ...
    public class ReportDeliveryNoteVatInvoiceController : ControllerBase
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IWorkContext _workContext;
        private readonly ITenantService _tenantService;

        public ReportDeliveryNoteVatInvoiceController(IUnitOfWork unitOfWork, IWorkContext workContext, ITenantService tenantService)
        {
        ...
        public async Task<IActionResult> DeliveryNotes(Guid deliveryNoteId)
        {
            // ==============================
            ...
            //Delivery Note
            var deliveryNoteVatInvoice = await _unitOfWork.Repository<DeliveryNote>().TableNoTracking().Include(x => x.Customer).Include(x => x.Store).Where(d =>
                                d.Id == deliveryNoteId).FirstOrDefaultAsync();

            if (deliveryNoteVatInvoice is not null)
            {
                String? customerCode = deliveryNoteVatInvoice.Customer.Code;
                String? customerName = deliveryNoteVatInvoice.Customer.Name;
                String? customerAddress = deliveryNoteVatInvoice.Customer.Address;
                String? customerContactNo = deliveryNoteVatInvoice.Customer.ContactNo;
                String? deliveryNoteNo = deliveryNoteVatInvoice.DeliveryNoteNo;
                String? deliveryDate = deliveryNoteVatInvoice.DeliveryDate.ToLocal().ToString("dd/MM/yyyy");
                String? createdOn = deliveryNoteVatInvoice.CreatedOn.ToLocal().ToString("hh:mm:ss tt");
                String? deliveryPlace = deliveryNoteVatInvoice.DeliveryPlace;
                String? truckNo = deliveryNoteVatInvoice.TruckNo;

                // ======================
                // Get Delivery Note Item
                // ======================
                var deliveryNoteVatInvoiceItems = _unitOfWork.Repository<DeliveryNoteDetail>().TableNoTracking().Include(x => x.Product).ThenInclude(x => x.MeasurementUnit).Where(d =>
                                       d.DeliveryNoteId == deliveryNoteId).Select(d => new
                                       {
                                           ...
                                           Quantity = d.DeliveryQuantity,
                                           NetRate = d.NetRate - d.MonthlyDiscountPerUnit - d.YearlyDiscountPerUnit - d.TargetDiscountPerUnit,
                                           NetAmount = d.DeliveryQuantity * (d.NetRate - d.MonthlyDiscountPerUnit - d.YearlyDiscountPerUnit - d.TargetDiscountPerUnit)
                ...
                PdfPTable supplierDeliveryNotesInfo = new(4);
                float[] widthsCellsSupplierDeliveryNotesInfo = new float[] { 22f, 42f, 18f, 18f };
                supplierDeliveryNotesInfo.SetWidths(widthsCellsSupplierDeliveryNotesInfo);
                supplierDeliveryNotesInfo.WidthPercentage = 100;
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("", fontArial8)) { Colspan = 4, Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("Name of Purchaser:", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 2 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase(customerName, fontArial8Bold)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("Challan No:", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 1f, PaddingRight = 2f, HorizontalAlignment = 2 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase(deliveryNoteNo, fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("Bin of Purchaser (Applicable Where):", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 2 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("Date of Issue:", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 1f, PaddingRight = 2f, HorizontalAlignment = 2 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase(deliveryDate, fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("Address of Purchaser:", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 2 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase(customerAddress, fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("Time of Issue:", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 1f, PaddingRight = 2f, HorizontalAlignment = 2 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase(createdOn, fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                if (!string.IsNullOrEmpty(customerContactNo))
                {
                    supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("", fontArial8)) { Border = 0, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                    supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase(customerContactNo, fontArial8)) { Colspan = 3, Border = 0, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                }
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("Destination of Supply:", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 2 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase(deliveryPlace, fontArial8)) { Colspan = 3, Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("Nature of Vehicle and Number:", fontArial8)) { Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 2 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase(truckNo, fontArial8)) { Colspan = 3, Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("", fontArial8)) { Colspan = 4, Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                supplierDeliveryNotesInfo.AddCell(new PdfPCell(new Phrase("", fontArial8)) { Colspan = 4, Border = 0, PaddingTop = 3f, PaddingLeft = 2f, PaddingRight = 2f, HorizontalAlignment = 0 });
                document.Add(supplierDeliveryNotesInfo);

                if (deliveryNoteVatInvoiceItems.Any())
                {
                    ...
                    tableVatInvoiceItems.AddCell(new PdfPCell(new Phrase("Quantity (KG)", fontArial7Bold)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 4f, VerticalAlignment = Element.ALIGN_MIDDLE });
                    ...
                    int sl = 0;
                    foreach (var deliveryNoteItem in deliveryNoteVatInvoiceItems)
                    {
                        sl++;
                        ...
                        tableVatInvoiceItems.AddCell(new PdfPCell(new Phrase(deliveryNoteItem?.ProductCode, fontArial7)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 3f });
                        tableVatInvoiceItems.AddCell(new PdfPCell(new Phrase(deliveryNoteItem?.ProductName, fontArial7)) { HorizontalAlignment = 0, PaddingTop = 3f, PaddingBottom = 3f });
                        tableVatInvoiceItems.AddCell(new PdfPCell(new Phrase(deliveryNoteItem?.UnitName, fontArial7)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 3f });
                        tableVatInvoiceItems.AddCell(new PdfPCell(new Phrase(deliveryNoteItem?.Quantity.ToString("#,##0.00"), fontArial7)) { HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 3f });
                        tableVatInvoiceItems.AddCell(new PdfPCell(new Phrase(deliveryNoteItem?.NetRate.ToString("#,##0.00"), fontArial7)) { HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 3f });
                        tableVatInvoiceItems.AddCell(new PdfPCell(new Phrase(deliveryNoteItem?.NetAmount.ToString("#,##0.00"), fontArial7)) { HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 3f });
                    ...
                    tableVatInvoiceItems.AddCell(new PdfPCell(new Phrase(deliveryNoteVatInvoiceItems.Sum(x => x.NetAmount).ToString("#,##0.00"), fontArial7Bold)) { HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 3f });
