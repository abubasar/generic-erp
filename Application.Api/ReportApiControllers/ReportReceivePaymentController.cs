...
namespace Application.Api.ReportApiControllers
{
    ...
    public class ReportReceivePaymentController : ControllerBase
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IWorkContext _workContext;
        private readonly ITenantService _tenantService;

        public ReportReceivePaymentController(IUnitOfWork unitOfWork, IWorkContext workContext, ITenantService tenantService)
        {
        ...
        public async Task<IActionResult> ReceivePayments(Guid receivePaymentId)
        {
            ...
            if (receivePayment is not null)
            {
                ...
                Decimal? feedSalesPurposeAmount = receivePayment.FeedSalesPurpose;
                Decimal? creditRecoveryPurposeAmount = receivePayment.CreditRecoveryPurpose;
                ...
                var receivePaymentItems = _unitOfWork.Repository<ReceivePaymentDetail>().TableNoTracking().Include(x => x.Account).Where(d =>
                                       d.ReceivePaymentId == receivePayment.Id).Select(d => new
                                       {
                                           d.Id,
                                           Code = d.Account.Code,
                ...
                if (totalAmount == feedSalesPurposeAmount) narrationMessage = "Receipt against Feed Sales.";
                else if (totalAmount == creditRecoveryPurposeAmount) narrationMessage = "Receipt against Credit Sales.";
                else narrationMessage = "Receipt against Feed Sales : " + feedSalesPurposeAmount?.ToString("#,##0.00") + ", Receipt against Credit Sales : " + creditRecoveryPurposeAmount?.ToString("#,##0.00");
                ...
                calculationTable.AddCell(new PdfPCell(new Phrase(narrationMessage, fontArial9)) { Border = 0, PaddingTop = 2f, PaddingLeft = 5f, PaddingRight = 5f, HorizontalAlignment = 0 });
