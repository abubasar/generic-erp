...
@Component({
  ...
  styleUrls: ["./supplier-transaction-report.component.scss"],
})
export class SupplierTransactionReportComponent implements OnInit {
  ...
  printPdf($event) {
    ...
    this.http
      .post(
        environment.apiURL + "/Account/supplier-transaction-ledger-print",
      ...
      .subscribe((response) => {
        if ($event?.submitter?.name == "pdf") {
          ...
          link.href = url;
          link.setAttribute("download", "supplier_transaction.xlsx");
