...
export class LcAdjustmentPurchaseInvoiceDialogComponent implements OnInit {
  ...
  getPurchaseInvoices(keyword: string): void {
    this.purchaseInvoiceRequest.purchaseInvoiceStatus = 3; // it will be uncomment after complete
    this.purchaseInvoiceRequest.keyword = keyword;
    this.purchaseInvoiceRequest.isImportPurchase = true;
    this.purchaseInvoiceService
      ...
      .subscribe((res) => {
        this.dataSource = new MatTableDataSource<PurchaseInvoiceResponseDTO>(
          res?.data?.item1?.filter((pi) => !pi.isLcAdjusted)
        );
  ...
  getPurchaseInvoiceNo(keyword): void {
    this.purchaseInvoiceRequest.purchaseInvoiceStatus = 3;
    this.purchaseInvoiceRequest.isImportPurchase = true;
    this.purchaseInvoiceRequest.keyword = keyword;
    this.purchaseInvoiceService
      .getPurchaseInvoices(this.purchaseInvoiceRequest)
      .subscribe((res) => {
        this.purchaseInvoices = res?.data?.item1?.filter(
          (pi) => !pi.isLcAdjusted
        );
      });
  }
  ...
  onPageChange(pageEvent) {
    ...
    this.getPurchaseInvoices(this.searchForm.get("purchaseInvoiceNo").value);
  }
