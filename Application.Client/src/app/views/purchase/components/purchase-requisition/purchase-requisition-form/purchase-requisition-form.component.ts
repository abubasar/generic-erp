...
export class PurchaseRequisitionFormComponent implements OnInit {
  ...
  public path = {
    list: "/purchase/purchase-requisition",
  ...
  initializeForm() {
    this.createForm();
    this.populateForm();
    this.purchaseRequisitionForm.get("requisitionDate").markAsTouched();
  }
  ...
  handleStoreSelection() {
    this.resetProductInPurchaseRequisitionDetails(
      this.purchaseRequisitionDetails
    );
  }

  resetProductInPurchaseRequisitionDetails(arr: FormArray) {
    for (let i = 0; i <= arr.length; i++) {
      const purchaseRequisitionDetail = arr.at(i);
      purchaseRequisitionDetail?.patchValue({
        productId: null,
      });
    }
  }
  ...
  clearInputFromDetails(evt: any, itemIndex: number): void {
    evt.stopPropagation();
    const particularDetail = this.purchaseRequisitionDetails.at(itemIndex);
    particularDetail?.patchValue({
      productId: null,
    });
  }
  ...
  //   this.getAllProducts(value.toLowerCase());
  // }

  private handleSuccessfulSave(res: GeneralResponse<string>): void {
    if (res?.succeeded) {
      this.handlePurchaseRequisitionResponse(res?.data);
  ...
  private handlePurchaseRequisitionResponse(
    purchaseRequisitionId: string
  ): void {
    this.purchaseRequisitionService
      .getPurchaseRequisitionById(purchaseRequisitionId)
      .subscribe({
        next: (purchaseRequisitionResponse) => {
          this.data = purchaseRequisitionResponse.data;
          this.isViewMode = true;
          this.isLoading = false;
          this.navigateToView(purchaseRequisitionResponse?.data?.id);
        },
        error: (err) => {
          location.reload();
        },
  ...
  onSubmit(): void {
    if (this.purchaseRequisitionForm.valid) {
      ...
      if (!formValue.id) {
        ...
        this.updatePurchaseRequisition(formValue);
      }
    }
  }
  ...
  check(id: string) {
    ...
    this.purchaseRequisitionService
      ...
      .subscribe({
        next: (res) => {
          if (res?.succeeded) {
            this.data.requisitionStatus = res?.data as number;
