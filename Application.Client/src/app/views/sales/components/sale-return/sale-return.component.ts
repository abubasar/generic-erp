...
export class SaleReturnComponent implements OnInit {
  ...
  expandedElement: any;
  dataSource: MatTableDataSource<SaleReturnResponseDTO>;
  reportAggregator = new SaleReturnAggregatorModel();
  totalCount: number;
  saleReturnRequest = new SaleReturnSearchRequestDTO();
  saleReturnStatuses: ENUM[];
  ...
  private path = {
    addNew: "sales/sale-return/add-new",
    edit: "sales/sale-return",
    view: "sales/sale-return/view",
  };
  ...
  columnDefinitions = [
    ...
    { def: "saleReturnNo", label: "Sale Return No." },
    { def: "saleReturnDate", label: "Sale Return Date" },
  ...
  ngOnInit(): void {
    ...
    this.getSaleReturns(this.saleReturnRequest);
  ...
  private initializeForm() {
    ...
    this.viewColumnForm = this.fb.group({
      ...
      customerId: [true],
      subtotal: [true],
      totalPercentageDiscountAmount: [true],
      otherDiscount: [true],
      total: [true],
      remark: [true],
      status: [true],
  ...
  getAllSaleReturnStatuses() {
    this.enumValueService.getSaleReturnStatuses().subscribe((res) => {
      this.saleReturnStatuses = res;
    });
  }

  getSaleReturnStatus(value: number) {
  ...
  getSaleReturns(requestBody: SaleReturnSearchRequestDTO): void {
    this.saleReturnService.getSaleReturns(requestBody).subscribe((res) => {
      this.dataSource = new MatTableDataSource<SaleReturnResponseDTO>(
        res?.data?.item1
      );
  ...
  reportAggregates(requestBody: SaleReturnSearchRequestDTO): void {
    this.saleReturnService.reportAggregates(requestBody).subscribe((res) => {
      this.reportAggregator = res.data;
    });
  }

  getPendingCheckedCount(): void {
    this.saleReturnService.getPendingCheckedCount().subscribe((res) => {
  ...
  getPending() {
    this.loading = true;
    this.saleReturnRequest.saleReturnStatus = 1;
    this.getSaleReturns(this.saleReturnRequest);
  }

  getChecked() {
    this.loading = true;
    this.saleReturnRequest.saleReturnStatus = 2;
    this.getSaleReturns(this.saleReturnRequest);
  }

  remove(id: string): void {
    this.saleReturnService.deleteSaleReturn(id).subscribe((res) => {
      if (res?.succeeded) {
        this.getSaleReturns(this.saleReturnRequest);
  ...
  reload() {
    ...
    this.loading = true;
    this.saleReturnRequest = new SaleReturnSearchRequestDTO();
    this.getSaleReturns(this.saleReturnRequest);
  }

  onSearch() {
    this.loading = true;
    this.saleReturnRequest = {
      ...this.saleReturnRequest,
  ...
  onPageChange(pageEvent) {
    this.saleReturnRequest.page = pageEvent.pageIndex;
    this.saleReturnRequest.rowsPerPage = pageEvent.pageSize;
    this.getSaleReturns(this.saleReturnRequest);
  }
  ...
  printSaleReturnPdf(name: string) {
    this.http
      .post(environment.apiURL + "/SaleReturn/" + name, this.searchForm.value, {
        responseType: "blob",
      })
      .subscribe((response) => {
        //Create a Blob from the PDF Stream
