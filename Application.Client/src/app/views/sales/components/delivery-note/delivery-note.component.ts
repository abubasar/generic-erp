...
export class DeliveryNoteComponent implements OnInit {
  ...
  reportAggregator = new DeliveryNoteAggregatorModel();
  totalCount: number;
  ...
  private path = {
    addNew: "sales/delivery-note/add-new",
    edit: "sales/delivery-note",
    view: "sales/delivery-note/view",
  };
  ...
  ngOnInit(): void {
    ...
    this.getDeliveryNotes(this.deliveryNoteRequest);
  ...
  openExpansionPanel() {
    ...
    this.getAllDeliveryNoteStatuses();
  ...
  getDeliveryNotes(requestBody: DeliveryNoteSearchRequestDTO): void {
    this.deliveryNoteService.getDeliveryNotes(requestBody).subscribe((res) => {
  ...
  reportAggregates(requestBody: DeliveryNoteSearchRequestDTO): void {
    this.deliveryNoteService.reportAggregates(requestBody).subscribe((res) => {
      this.reportAggregator = res.data;
    });
  }

  getPendingCheckedCount(): void {
    this.deliveryNoteService.getPendingCheckedCount().subscribe((res) => {
  ...
  getPending() {
    this.loading = true;
    this.deliveryNoteRequest.deliveryNoteStatus = 1;
    this.getDeliveryNotes(this.deliveryNoteRequest);
  }

  getChecked() {
    this.loading = true;
    this.deliveryNoteRequest.deliveryNoteStatus = 2;
    this.getDeliveryNotes(this.deliveryNoteRequest);
  }

  remove(id: string): void {
    this.deliveryNoteService.deleteDeliveryNote(id).subscribe((res) => {
      if (res?.succeeded) {
        this.getDeliveryNotes(this.deliveryNoteRequest);
  ...
  reload() {
    ...
    this.loading = true;
    this.deliveryNoteRequest = new DeliveryNoteSearchRequestDTO();
    this.getDeliveryNotes(this.deliveryNoteRequest);
  }
  ...
  printDeliveryNotePdf(name: string) {
    this.http
      .post(
        environment.apiURL + "/DeliveryNote/" + name,
        this.searchForm.value,
        {
          responseType: "blob",
        }
