...
| `Application.Signup` | Standalone Angular 20 public self-service signup wizard (separate app, no auth) |
...
**Shared rows (`ITenantSharable`)**: `Account`, `AccountType`, `InventoryType`, `PaymentMode`, `ProductType` implement `ITenantSharable : ITenantScoped` — the query filter matches `TenantId == current OR TenantId == TenancyConstants.SystemTenantId` (`ffffffff-…`). The code hard-codes GUIDs for these (`AccountHeadConstants`, `AccountTypeConstants`, `InventoryTypeConstants`, `PaymentModeConstants`, `ProductTypeConstant`); those constant rows — for `Account`, the 46 head GUIDs + 10 intermediate groups up to the roots (55 rows) — carry `SystemTenantId`, so every tenant resolves the same GUIDs while its own customer/supplier/custom rows stay tenant-scoped. `UnitOfWork` never re-stamps or cross-tenant-guards a `SystemTenantId` row. Migrations `ShareSystemAccountHeads`, `ShareSystemLookups`.

**Per-tenant unique codes**: `Account.Code`, `FinancialYear.Code`, `Store.Code`, `Company.Code`, `Product.Code`, `PurchaseOrder.PONumber`, `SaleOrder.SaleOrderNo` were DB-first GLOBAL unique constraints (a 2nd tenant couldn't create them); now unique indexes on `(TenantId, <code>)`. Migrations `ShareSystemAccountHeads`, `PerTenantUniqueCodes`. When adding such a constraint, always scope it by `TenantId`.

**Tenant provisioning (Phase 2)**: `ProvisioningService` (`Application.Services/Services/Platform`) — idempotent, keyed on `ProvisioningStep(TenantId, StepKey)`. Steps: subdomain, subscription (Trial), financial-year (BD fiscal Jul–Jun), measurement-units, company, default-store, owner-role (+ all `RoleClaim`s from `ClaimsHelper`), owner-user (generated password returned once). `PlatformTenantService.CreateAsync` provisions after create and returns `CreateTenantResult`. `POST /api/platform/tenants/{id}/provision` re-runs; `GET .../provisioning` is the failed-step queue. A new tenant is fully usable (login works) with no manual setup.

**Pricing (Phase 2)**: `PricingEngine` — `POST /api/platform/pricing/quote` returns an itemised monthly quote from the published `PriceBook`. Entry item types: `base` (one), `plan:<key>`, `module:<key>`, `unit:<quotaKey>` (`UnitPrice` = per extra over the plan's included quota).
...
**Public signup surface (Phase 2 — plan §06)**: `Application.Api/Controllers/SignupController.cs` at `api/signup` — deliberately no auth attribute (kept apart from the `[PlatformAuthorize]`-gated `Controllers/Platform/*`), reachable by any anonymous visitor. `GET business-templates`/`plans` return only `IsPublic` (and, for plans, `IsActive`) catalog rows; `POST pricing/quote` reuses `IPricingEngine` after checking the plan is public; `POST tenants` calls `PlatformTenantService.SignUpAsync(PublicSignupRequest)`, which validates the template/plan are public, generates a unique `Tenant.Code` from the business name (capped short — see below), and delegates to the existing `CreateAsync`/`ProvisioningService` pipeline. `Tenant.Code` is `HasMaxLength(10)`, a pre-multi-tenancy column never widened — any generated code must fit that. Three plans (Starter/Business/Enterprise) and a published `PriceBook` are seeded via migration `SeedSignupPlansAndPriceBook`; all three grant the same 7 modules and differ only by `max_users`/`max_branches`/`max_pos_terminals` quotas, since Pharma/Feed both need all 7 modules today and modules aren't a real per-plan lever yet. The wizard UI is the separate `Application.Signup` app (`npm install && npm start`, serves on :4400, proxies `/api` to :5254).

**Manual invoicing (Phase 2 — plan §08, "invoicing stays manual: your team issues the bill and marks it paid")**: `PlatformInvoice` (`Application.Core/Entities/Platform/`) — TenantId, Number (auto, `INV-<year>-<seq>`), PeriodStart/End, Amount, Currency, Status (`Unpaid`/`Paid`/`Void`), IssuedOn, DueOn, PaidOn, Note. Deliberately not called `Invoice` (clashes with `SaleInvoice`/`PurchaseInvoice`) and deliberately without line items — the itemised, `PricingEngine`-driven `TenantInvoice`/`TenantInvoiceLine` is Phase 4 (automated billing), not this. `IPlatformInvoiceService` (`Application.Services/Services/Platform/`): `ListAsync`/`CreateAsync`/`MarkPaidAsync`/`VoidAsync`; a paid invoice can't be voided and a void one can't be marked paid. Endpoints nested under `PlatformTenantsController`: `GET/POST /api/platform/tenants/{id}/invoices`, `POST .../invoices/{invoiceId}/paid`, `POST .../invoices/{invoiceId}/void` — all `[PlatformAuthorize]`, mutations need `Admin`. PlatformConsole's tenant-detail page has an Invoices card (list + create form + mark-paid/void buttons).
